...
def intermediate_node_num_mistakes(labels_in_node):
    ...
    trues = sum(labels_in_node==+1)
    
    # Count the number of -1's (risky loans)
    falses = len(labels_in_node)-trues
                
    # Return the number of mistakes that the majority classifier makes.
    return min(trues,falses)
...
def best_splitting_feature(data, features, target):
    ...
    for feature in features:
        ...
        #print feature,left_mistakes,right_mistakes

        # If this is the best error we have found so far, store the feature as best_feature and the error as best_error
        #if error < best_error or (error == best_error and min(len(left_split),len(right_split)) > 0):
        if error < best_error:
            best_error = error
            best_feature = feature
    
    #print 'and the best is ....',best_feature,best_error
...
def evaluate_classification_error(tree, data):
    ...
    print sum(prediction <> data[target])/float(len(data))
